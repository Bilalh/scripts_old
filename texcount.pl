...
my $versionnumber="2.3.alpha.950";
my $versiondate="2011 Feb 12";
...
sub parse_options_parsing {
  ...
  elsif ($arg=~/^-(alpha(bets?)?)=(.*)$/) {
    set_script_options(\@AlphabetScripts,$3);
  }
  elsif ($arg=~/^-(logo(grams?)?)=(.*)$/) {
    set_script_options(\@LogogramScripts,$3);
  }
...
sub _parse_unit {
  ...
  while (defined (my $next=_next_token($tex))) {
    ...
    if ((defined $end) && ($end eq $next)) {
      ...
      print_error($tex,"ERROR: End of file while waiting for ".$end);
      return;
    }
...
sub _parse_tc {
  ...
  $next=~s/^\%+TC:\s*(\w+)\s*//i || BLOCK {
    print_error($tex,'Warning: TC command should have format %TC:instruction [macro] [parameters]');
    return;
  };
  my $instr=$1;
  $instr=~tr/[A-Z]/[a-z]/;
  if ($instr=~/^(break)$/) {
    ...
    if ($next=~/^(\\\S+)\s+/) {
      ...
      print_error($tex,"Warning: Invalid TC:subst format.");
    }
  } elsif ($next=~/^([\\]*\S+)\s+([^\s\n]+)(\s+(-?[0-9]+))?/) {
    ...
    else {print_error($tex,"Warning: Unknown TC command: ".$instr);}
  } else {
    print_error($tex,"Warning: Invalid TC command format: ".$instr);
  }
}
...
sub _parse_begin_end {
  ...
  if ($tex->{'line'} =~ s/^\{([^\{\}\s]+)\}[ \t\r\f]*//) {
    ...
    print_style('{'.$1.'}',$localstyle);
    if ($groupname ne $1) {
      print_error($tex,"Warning: begin\{".$groupname."\} ended with end\{".$1."\}.");
    }
  } else {
    print_error($tex,"Warning: END group without type while waiting to end ".$groupname.".");
  }
}
...
sub _parse_include_file {
  ...
  || $tex->{'line'} =~ s/^\s*\{(.+?)\}//
  || BLOCK {
    print_error($tex,"WARNING: Failed to read or interpret file name for inclusion.");
    return;
  };
  if ($status>0) {
